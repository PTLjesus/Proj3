...
void square_sgemm_manager1(int n, float* A, float* B, float* C)
{
	...
	for(int iii = 0; iii < n; iii += BLOCK_SIZE1)
		for(int jjj = 0; jjj < n; jjj+= BLOCK_SIZE1)
		
			for(int ii = iii; ii < BLOCK_SIZE1 && ii < n  ; ii += BLOCK_SIZE)
				for(int jj = jjj; jj < BLOCK_SIZE1 && jj < n; jj += BLOCK_SIZE)
	
					for( int i = ii; i < ii + BLOCK_SIZE && i < n ; i += 4 ) //right by 4
						for( int j = jj; j < jj+ BLOCK_SIZE && j < n ; j += 4 ) //down by 4
